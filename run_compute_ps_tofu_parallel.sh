...
# Configuration for TOFU dataset
MODEL_DIR="/scratch/aebrahim/muse_rep/finetuned_tofu_llama2_model/"
TOKENIZER_DIR="meta-llama/Llama-2-7b-hf"

# TOFU-specific paths
SPLIT_THRESHOLD=""  # Set to "2p0" if using split answers, empty otherwise
if [ -z "$SPLIT_THRESHOLD" ]; then
    SAMPLES_CSV="site_outputs_tofu_/samples.csv"
    CORRUPTIONS_CSV="corruptions_tofu_llama2/tofu_corruptions.csv"
else
    SAMPLES_CSV="site_outputs_tofu_split_${SPLIT_THRESHOLD}x/samples.csv"
    CORRUPTIONS_CSV="corruptions_tofu/tofu_corruptions_split_${SPLIT_THRESHOLD}x.csv"
fi

PROMPTS_CSV="tofu_data/tofu_full_train.csv"
OUTPUT_DIR="ps_outputs_tofu"
...
for i in $(seq 0 $(($NUM_GPUS - 1))); do
    ...
    if [ -f "$SUBSET_SAMPLES" ]; then
        ...
        # Build the command with --tofu_format flag
