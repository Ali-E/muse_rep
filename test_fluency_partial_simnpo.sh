...
for forget_portion in "${forget_portion_list[@]}"; do
    ...
    if [ "$forget_portion" == "1.0" ]; then
        model_dir="/scratch/aebrahim/muse_rep/Llama2_ft_mine/ckpt/$CORPUS/${algo}_b${beta}_g${gamma}_${forget_portion}_UW_2x_s${SEED}/"
    else
        model_dir="/scratch/aebrahim/muse_rep/Llama2_ft_mine/ckpt/$CORPUS/${algo}_b${beta}_g${gamma}_${forget_portion}_UW_2x_s${SEED}/"
    fi
    ...
    output_file="$RESULTS_DIR/${algo}_b${beta}_g${gamma}_UW_2x_${forget_portion}_fluency.json"
